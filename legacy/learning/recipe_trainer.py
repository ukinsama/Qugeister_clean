...
def train_specific_recipe(recipe_name, epochs=None):
    """指定レシピの学習"""
    # レシピファイルを探す
    possible_paths = [
        f"../{recipe_name}",
        f"../{recipe_name}.py",
        f"../quick_{recipe_name}.py",
        f"../{recipe_name}_recipe.py",
        f"../{recipe_name}_integrated_recipe.py"
    ]
    ...
    if not recipe_path:
        print(f"❌ レシピが見つかりません: {recipe_name}")
        print("💡 利用可能なレシピ:")
        recipes = find_ai_recipes()
        for recipe in recipes:
            print(f"   - {Path(recipe).stem}")
        return None
    
    print(f"📚 個別学習: {recipe_path}")
    if epochs:
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == '--batch':
            ...
            if len(sys.argv) < 3:
                print("❌ レシピ名を指定してください")
                print("💡 使用方法: python recipe_trainer.py --recipe test06 100")
                return
            ...
            if len(sys.argv) > 3:
                try:
                    ...
                    print(f"❌ 無効なエポック数: {sys.argv[3]}")
                    return
            
            model_path = train_specific_recipe(recipe_name, epochs)
            if model_path:
                print(f"✅ 個別学習完了: {model_path}")
...
🔍 個別レシピ学習:
   python learning/recipe_trainer.py --recipe test06
   python learning/recipe_trainer.py --recipe test06 100  # エポック数指定
