...
class TournamentManager:
    ...
    def discover_and_load_models(self):
        ...
        for search_dir in search_dirs:
            try:
                loader = TrainedModelLoader(search_dir)
                models = loader.discover_models()
                found_models.extend(models)
                print(f"📁 {search_dir}: {len(models)}個発見")
            except Exception as e:
                continue
        
        print(f"📁 発見された学習済みモデル: {len(found_models)}個")
        
        for model_metadata in found_models:
            model_id = model_metadata['model_id']
            try:
                # モデルのディレクトリからローダーを作成
                model_dir = model_metadata.get('model_path', '')
                model_loader = TrainedModelLoader(os.path.dirname(model_dir))
                model_wrapper = model_loader.load_model(model_id)
                if model_wrapper:
