...
class GeisterStateEncoder:
    ...
    def encode_full_state(
        self, board_state, player_pieces, opponent_pieces, legal_moves, escape_positions
    ):
...
class QValueFullOutputModule:
    ...
    def generate_all_possible_states(self, max_states=1000):
        ...
        for i in range(max_states):
            ...
            # 相手の駒をランダム配置
            opponent_positions = [
                (x, y) for x in range(6) for y in range(0, 2)
            ]  # 上側2行
            selected_opp = np.random.choice(
                len(opponent_positions), size=4, replace=False
            )
    ...
    def compute_full_qvalue_map(self, states, state_descriptions):
        ...
        with torch.no_grad():
            for i, (state, desc) in enumerate(zip(states, state_descriptions)):
                ...
                qvalue_map_36d = (
                    self.model(state_tensor).squeeze().numpy()
                )  # 36次元出力
                ...
                # 5行動Q値を計算
                action_qvalues = (
                    self.model.get_action_from_qmap(state_tensor).squeeze().numpy()
                )
                ...
                if (i + 1) % 100 == 0:
                    print(
                        f"   進捗: {i+1}/{len(states)} ({100*(i+1)/len(states):.1f}%)"
                    )
        ...
        statistics = {
            ...
            "qvalue_map_stats": {
                ...
                "range": [
                    float(np.min(all_qvalue_maps)),
                    float(np.max(all_qvalue_maps)),
                ],
                "spatial_variance": float(
                    np.mean([np.var(qmap.reshape(6, 6)) for qmap in all_qvalue_maps])
                ),
            },
            ...
            "action_qvalue_stats": {
                ...
                "range": [
                    float(np.min(all_action_qvalues)),
                    float(np.max(all_action_qvalues)),
                ],
            },
        }
    ...
    def visualize_qvalue_heatmap(self, all_qvalues, save_path=None):
        ...
        bars = ax_stats.bar(
            range(5), action_counts, color=["C0", "C1", "C2", "C3", "C4"]
        )
    ...
    def visualize_action_vs_spatial_comparison(
        self, qvalue_map, all_action_qvalues, save_path=None
    ):
        ...
        # 2. 空間Q値の位置別平均
        all_spatial_maps = np.array(
            [data["qvalue_spatial"] for data in qvalue_map.values()]
        )
        ...
        ax4.set_xticklabels(
            [f"({pos//6},{pos%6})" for pos in top_positions], rotation=45
        )
    ...
    def export_results(
        self, qvalue_map, statistics, pattern_stats, all_qvalue_maps, all_action_qvalues
    ):
        """結果をファイルに出力"""
        ...
        with open(csv_path, "w") as f:
            f.write(
                "state_id,best_action,max_qvalue,qvalue_variance,legal_moves_count\\n"
            )
            for i, data in qvalue_map.items():
                f.write(
                    f"{i},{data['best_action']},{data['max_qvalue']:.4f},"
        ...
        self.visualize_action_vs_spatial_comparison(
            qvalue_map, all_action_qvalues, heatmap_path
        )
    ...
    def run_full_analysis(self, num_states=1000):
        ...
        # ステップ2: Q値計算
        qvalue_map, statistics, all_qvalue_maps, all_action_qvalues = (
            self.compute_full_qvalue_map(states, state_descriptions)
        )
        ...
        print(
            f"  36次元Q値範囲: [{statistics['qvalue_map_stats']['range'][0]:.3f}, {statistics['qvalue_map_stats']['range'][1]:.3f}]"
        )
        print(
            f"  36次元Q値平均: {statistics['qvalue_map_stats']['mean']:.3f} ± {statistics['qvalue_map_stats']['std']:.3f}"
        )
        print(
            f"  空間分散平均: {statistics['qvalue_map_stats']['spatial_variance']:.4f}"
        )

        print("\\n📊 5行動Q値統計:")
        print(
            f"  5行動Q値範囲: [{statistics['action_qvalue_stats']['range'][0]:.3f}, {statistics['action_qvalue_stats']['range'][1]:.3f}]"
        )
        print(
            f"  5行動Q値平均: {statistics['action_qvalue_stats']['mean']:.3f} ± {statistics['action_qvalue_stats']['std']:.3f}"
        )
        ...
        for i, (name, count) in enumerate(
            zip(action_names, statistics["action_distribution"])
        ):
        ...
        # ステップ5: 結果出力
        self.export_results(
            qvalue_map, statistics, pattern_stats, all_qvalue_maps, all_action_qvalues
        )
